/**
 * 人物画像视图脚本 (PersonProfile.js) v4.0 - ViewQuery Dataset
 *
 * 功能：
 * 1. 聚合与当前人物相关的所有“事情”和“往来”。
 * 2. [Fix] 修复事件视图的日期筛选失效问题。
 * 3. 保持 v3.3 的所有功能（导出、分栏、视觉升级）。
 */
...
/* 仪表盘 */
.pp-dashboard {
    display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-bottom: 12px; flex-shrink: 0;
}
.pp-kpi-card {
...
.pp-badge-time { background: rgba(112, 48, 160, 0.1); color: var(--c-accent); }
...
const viewType = input.type || "event";
let items = [];
...
// 生日信息
const birthdayInfo = Utils.getBirthdayInfo(dv.current());
...
function isCurrentPersonLink(link) {
...
function makePersonProfileConsume() {
    return {
...
function collectPersonProfileDataset(interaction = {}) {
    const dataset = ViewQuery.collect({
        Query,
        ViewKit,
        source: { querySources: { linkedTo: true } },
        rules: queryRules,
        consume: makePersonProfileConsume(),
        interaction,
        excludeLink: isCurrentPersonLink,
    });
    if (input && typeof input === "object") input.personProfileDataset = dataset;
    return dataset;
}
...
let currentDataset = collectPersonProfileDataset();
...
if (viewType === "event") {
    ...
    function renderEventView(data) {
        ...
        let birthdayHtml = "";
        if (birthdayInfo && !birthdayInfo.error) {
            const days = birthdayInfo.days;
            const dateStr = birthdayInfo.original ? birthdayInfo.original.toFormat("MM-dd") : "";
            birthdayHtml = `
                <div style="font-size:0.8em; color:var(--c-text-muted)">${dateStr}</div>
                <div style="font-weight:bold; color:${days < 30 ? 'var(--c-danger)' : 'var(--c-text-normal)'}">还有 ${days} 天</div>
             `;
        } else {
            birthdayHtml = `<div style="font-size:0.8em; color:var(--c-text-muted)">未设置</div>`;
        }

        dash.createEl('div', { cls: 'pp-kpi-card' }).innerHTML = `
            <div class="pp-kpi-icon">⏳</div>
            <div class="pp-kpi-val">${totalTime.toFixed(1)}🍅</div>
            <div class="pp-kpi-label">累计陪伴</div>
        `;
        dash.createEl('div', { cls: 'pp-kpi-card' }).innerHTML = `
        ...
        dash.createEl('div', { cls: 'pp-kpi-card' }).innerHTML = `
            <div class="pp-kpi-icon">🎂</div>
            ${birthdayHtml}
            <div class="pp-kpi-label">生日倒数</div>
        `;
        dash.createEl('div', { cls: 'pp-kpi-card' }).innerHTML = `
            <div class="pp-kpi-icon">📅</div>
            <div class="pp-kpi-val">${data.length}</div>
            <div class="pp-kpi-label">互动次数</div>
        `;
    ...
    new ViewKit.FilterBar(filterHost, {
        ...
        storageKey: `PersonProfile:v2:event:${targetFile.path}`,
        onFilter: (_filtered, state) => {
            currentDataset = collectPersonProfileDataset(state);
            items = datasetToProfileItems(currentDataset);
    ...
    const txDirectionLabels = { all: "全部往来", income: "收入/借入", expense: "支出/借出" };
    ...
    const txTitle = txTitleWrap.createEl('div', { cls: 'tx-section-title', text: '全部往来' });
    const txMeta = txTitleWrap.createEl('div', { cls: 'tx-section-meta', text: '默认显示全部往来' });
    ...
    const getDirectionText = amount => amount > 0 ? "收入/借入" : amount < 0 ? "支出/借出" : "零额";
    ...
    function renderDirectionTabs(counts) {
        ViewKit.renderSegmentedControl(directionHost, {
            ...
            options: [
                ...
                { key: "income", label: "收入/借入", count: counts.income },
                { key: "expense", label: "支出/借出", count: counts.expense },
            ],
            onChange(next) {
                txDirection = next;
    ...
    new ViewKit.FilterBar(filterHost, {
        ...
        initial: { startDate: startDateStr || '', endDate: endDateStr || '', sort: 'date', sortAsc: false },
        storageKey: `PersonProfile:v2:transaction:${targetFile.path}`,
        onFilter: (_filtered, state) => {
            currentDataset = collectPersonProfileDataset(state);
            items = datasetToProfileItems(currentDataset);
