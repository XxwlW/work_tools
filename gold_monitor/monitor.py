...
CONFIG_FILE = "/media/disk1/glod_project/gold_monitor/config.json"
...
def main():
    ...
    while True:
        ...
        if time.time() - last_change_time > TIMEOUT_MINUTES * 60:
            logger.info(f"超过 {TIMEOUT_MINUTES} 分钟价格无变化，退出监控")
            sys.exit(0)
