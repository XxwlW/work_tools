...
class MatplotlibRenderer:
    ...
    def __init__(self, figure_width: int = 12, figure_height: int = 9):
        ...
        self._annotation: Text | None = None
        self._hover_annotation: Text | None = None  # 悬停提示 by _show_hover_at (旧)
        ...
        self._source_status: plt.Text | None = None  # 来源组状态栏
    ...
    def _hide_hover(self) -> None:
        ...
        if self._annotation and self._annotation.get_visible():
            return  # 有固定选中时不覆盖
        self._last_hover_key = None
        self._status_text.set_text("左键点击查看坐标 | 右键拖动 | 滚轮缩放 | F/P/S 切换传感器 | R 重置")
        self.fig.canvas.draw()
    ...
    def set_source_status(self, status_text: str) -> None:
        """设置底部来源组状态"""
        if self._source_status is not None:
            self._source_status.set_text(status_text)
            self.fig.canvas.draw_idle()
    ...
    def _on_motion(self, event) -> None:
        ...
        if event.inaxes != self.ax:
            if self._last_hover_key is not None:
                self._hide_hover()
            return
    ...
    def set_key_handler(self, handler) -> None:
        ...
        注意：matplotlib TkAgg 的 key_press_event 在 TextBox/Slider
        获得焦点后不会触发字母键。因此额外绑定 Tk 窗口级按键。
        """
        if self.fig is None:
            return
        # matplotlib 级绑定（方向键/空格等可正常触发）
        self.fig.canvas.mpl_connect("key_press_event", handler)
        # Tk 窗口级绑定（TextBox 有焦点时字母键也能触发）
        try:
            tk_window = self.fig.canvas.manager.window
            tk_window.bind_all("<Key>", lambda e: self._on_tk_key(e, handler))
        except Exception:
            pass

    def _on_tk_key(self, tk_event, handler):
        """Tk 窗口级键盘事件转发"""
        try:
            key = tk_event.char
            if not key:
                return
            # 构造一个假的 matplotlib KeyEvent
            from matplotlib.backend_bases import KeyEvent
            fake_event = KeyEvent("key_press_event", self.fig.canvas, key)
            fake_event.key = key
            handler(fake_event)
        except Exception:
            pass
    ...
    def _apply_layout(self, ax: plt.Axes, fig: plt.Figure, title: str, legend_handles: dict) -> None:
        ...
        self._status_text = fig.text(
            0.5, 0.095,
            "左键点击查看坐标 | 右键拖动 | 滚轮缩放 | F/P/S 切换传感器 | R 重置",
            ha='center', va='bottom', fontsize=11, fontweight="bold",
            color='#222222',
        )
        # 来源组状态栏（显示 FSD/PDC/SDG 开关状态）
        if self._source_status is not None:
            self._source_status.remove()
        self._source_status = fig.text(
            0.5, 0.076,
            "[F:ON FSD] [P:ON PDC] [S:ON SDG]",
