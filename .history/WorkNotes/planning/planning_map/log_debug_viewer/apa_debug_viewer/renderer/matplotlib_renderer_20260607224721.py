...
class MatplotlibRenderer:
    ...
    def __init__(self, figure_width: int = 12, figure_height: int = 9):
        ...
        self._hover_threshold: float = 2.0  # 增大阈值到2米
    ...
    def setup_frame_selector(
        ...
        self._frame_text = TextBox(text_ax, '', initial=f'{initial_idx}')
        self._frame_text.label.set_fontsize(0)  # 隐藏标签，节省空间
        self._frame_text.on_submit(self._on_jump_to_frame)

        # 显示当前帧信息 (seq / total)
        self._frame_info_ax = self.fig.text(
            0.50, 0.062,
            f'Frame {initial_idx+1}/{total_frames}',
    ...
    def _on_slider_changed(self, val: int) -> None:
        ...
        if self._frame_text is not None:
            self._frame_text.set_val(f'{idx}')
        if self._frame_info_ax is not None:
            self._frame_info_ax.set_text(f'Frame {idx+1}/{self._total_frames}')
    ...
    def _on_jump_to_frame(self, text: str) -> None:
        ...
        try:
            idx = int(text.strip())
            if 0 <= idx < self._total_frames:
                self._on_frame_change(idx)
            else:
                print(f"帧索引 {idx} 超出范围 [0, {self._total_frames-1}]")
        except ValueError:
            print(f"无效的帧号: {text}，请输入数字 (0-{self._total_frames-1})")
    ...
    def _setup_annotation(self) -> None:
        ...
        self._annotation = self.ax.annotate(
            ...
            bbox=dict(boxstyle="round,pad=0.5", facecolor="lightyellow",
                      edgecolor="darkorange", alpha=0.95, linewidth=2),
            arrowprops=dict(
    ...
    def _find_nearest_point(self, x: float, y: float):
        ...
        for obj in self._scene_objects:
            if not obj.visible or not obj.points:
                continue
        ...
        # 如果最近点在阈值内，返回它
        if min_dist < self._hover_threshold:
            return nearest
        return None
    ...
    def _on_hover(self, event) -> None:
        ...
        if self._annotation is None or self.ax is None:
            return

        # 如果在 slider 区域，不处理悬停
        if self._frame_slider_ax is not None and event.inaxes == self._frame_slider_ax:
        ...
        result = self._find_nearest_point(event.xdata, event.ydata)
        ...
        lines = [
            f"({pt.x:.3f}, {pt.y:.3f}) m",
            f"{meta.get('point_type', obj.layer)}",
            f"{meta.get('source', '')}",
        ]
        ...
        self._annotation.xy = (pt.x, pt.y)
        self._annotation.xytext = (25, 25)
    ...
    def _on_button_press(self, event) -> None:
        """鼠标按下（右键平移）"""
        if event.button == 3 and event.inaxes == self.ax and self.ax:
    ...
    def _on_motion(self, event) -> None:
        """鼠标移动（平移）"""
        if not self._pan_state["dragging"] or event.inaxes != self.ax or self.ax is None:
            return
        if event.xdata is None or event.ydata is None:
            return
    ...
    def _draw_object(self, ax: plt.Axes, obj: SceneObject, legend_handles: dict) -> None:
        ...
        ax.plot(xs, ys, color=obj.color, linewidth=obj.line_width,
                alpha=obj.opacity, marker=marker, markersize=5, linestyle="-", label=obj.id)
        if obj.layer in ("CurrentVehicle", "GoalVehicle") and len(xs) > 4:
            poly = Polygon(list(zip(xs, ys)), closed=True, color=obj.color, alpha=0.15)
            ax.add_patch(poly)
        if obj.layer not in legend_handles:
            legend_handles[obj.layer] = Line2D([0], [0], color=obj.color,
                                                linewidth=obj.line_width, label=obj.layer)
