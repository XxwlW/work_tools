...
class MatplotlibRenderer:
    ...
    def _setup_annotation(self) -> None:
        """创建两个 annotation：悬停(细箭头) 和 点击固定(粗边框)"""
        if self.ax is None:
            return
        # 悬停提示 - 轻量，鼠标移入时显示
    ...
    def _show_hover_at(self, pt, obj) -> None:
        ...
        self._hover_annotation.xy = (pt.x, pt.y)
        self._hover_annotation.set_visible(True)
        # 如果有点击固定的 annotation，不覆盖它
    ...
    def _show_click_annotation(self, pt, obj) -> None:
        ...
        if self._status_text is not None:
            self._status_text.set_text(f" ● 选中: {text.replace(chr(10), ' | ')}")
        self.fig.canvas.draw_idle()
    ...
    def _setup_event_handlers(self) -> None:
        ...
        # pick 事件 - 鼠标滑过点时自动触发（比 motion_notify 更可靠）
    ...
    def _on_button_press(self, event) -> None:
        """鼠标按下：
          - 左键 (1) 点击固定选中最近点 / 点击空白处取消选中
          - 右键 (3) 平移画布
        """
        if event.button == 1:
            if event.inaxes == self.ax:
                # 左键点击画布：隐藏上次固定提示；如果 pick 命中点会重新显示
    ...
    def _on_pick(self, event) -> None:
        """picker 事件 — 鼠标靠近点时自动触发（picker=8 像素半径）

        行为：
          - 悬停（鼠标自然移动）→ 显示轻量悬停提示
          - 左键点击 → 固定显示选中提示（persist 直到下次点击空白处）
        """
        try:
            artist = event.artist
    ...
    def _on_motion(self, event) -> None:
        """鼠标移动（平移拖动 + 清理左键点击标记）"""
        # 鼠标移动时清理左键点击标记（避免空点击后下一帧误触发）
        if self._left_click_pending:
            self._left_click_pending = False

        if not self._pan_state["dragging"] or event.inaxes != self.ax or self.ax is None:
            # 非拖动状态：鼠标离开画布时隐藏悬停提示
            if event.inaxes != self.ax and self._hover_annotation and self._hover_annotation.get_visible():
                self._hide_hover()
            return
