...
class App:
    ...
    def run_gui(self) -> None:
        ...
        def on_key_press(event):
            ...
            elif event.key in ("f", "p", "s"):
                src = {"f": "FSD", "p": "PDC", "s": "SDG"}[event.key]
                status = self.layer_manager.toggle_source(src)
                print(f"  {status}")
                self._on_frame_changed()
                # 在底部状态栏显示当前 source 状态
                self.renderer.set_source_status(self.layer_manager.get_source_status())
            else:
                handled = False
